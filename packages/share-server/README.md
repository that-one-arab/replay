...
file or your platform's log drain. `REC_SHARE_LOG_LEVEL` (default `info`) tunes
verbosity.
...
```text
REC_SHARE_STATS_TOKEN=<random-token>       # enables GET /stats behind this bearer
```

```sh
curl -H "authorization: Bearer $REC_SHARE_STATS_TOKEN" https://<domain>/stats
```
...
path (`POST /v1/recordings`) than for reads. Over-limit requests get `429` with a
`Retry-After` header; `GET /health` is exempt. Defaults (all overridable):

```text
REC_SHARE_RATE_LIMIT_POINTS=120            # general requests per window
REC_SHARE_RATE_LIMIT_DURATION=60           # window, seconds
REC_SHARE_UPLOAD_RATE_LIMIT_POINTS=12      # uploads per window
REC_SHARE_UPLOAD_RATE_LIMIT_DURATION=60
REC_SHARE_TRUST_PROXY=false                # key on X-Forwarded-For when behind a proxy
```
...
behind a reverse proxy, set `REC_SHARE_TRUST_PROXY=true` so limiting keys on the
