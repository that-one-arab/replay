...
/**
 * The replay chat backend. One ChatSession per open player panel; each user
 * turn spawns the Codex CLI (`codex exec --json`) as the provider, resuming
 * the same Codex thread across turns. Codex reaches back into the replay
 * through a stdio MCP bridge that proxies tool calls to this manager, and
...
type ActiveTurn = { child: ChildProcess; canceled: boolean; stallTimer?: NodeJS.Timeout; sawCompletion: boolean; stderr: string };
...
export class ChatManager {
  ...
  /** Availability is config + a resolvable provider binary; auth surfaces per-turn. */
  async availability() {
    const config = await this.config();
    if (!config.enabled) return { available: false, provider: "codex", reason: "disabled" };
  ...
  connect(chatId: string, recordingId: string, response: ServerResponse) {
    ...
    sendEvent(response, "ready", { chat_id: chat.id, provider: "codex", busy: Boolean(chat.turn) });
  ...
  async message(chatId: string, text: string) {
    ...
    const prompt = chat.threadId ? clean : `${await this.firstTurnPreamble(chat)}\n\nViewer's question: ${clean}`;
    await this.runTurn(chat, config, prompt);
  ...
  dispose() {
    for (const chat of this.chats.values()) {
      if (chat.turn) stopChild(chat.turn.child);
  ...
  private record(chat: ChatSession, event: ChatEvent) {
    ...
    for (const client of chat.clients) sendEvent(client, event.type === "user_message" ? "user_message" : event.type, event);
  }

  private async firstTurnPreamble(chat: ChatSession) {
    const summary = await this.summaryFor(chat);
    return [
      ...
      "",
      "You have rec_replay tools to inspect and control the player:",
  ...
  private async runTurn(chat: ChatSession, config: ChatConfig, prompt: string) {
    ...
    const args = [
      ...
      ...(chat.threadId ? [] : ["-C", sessionPath(chat.recordingId), ...(config.model ? ["-m", config.model] : [])]),
      prompt,
    ];
    let child: ChildProcess;
    try {
      child = spawn(config.command, args, { stdio: ["ignore", "pipe", "pipe"], env: process.env });
    } catch (error) {
      this.finishTurn(chat, "failed", messageOf(error), "spawn_failed");
      return;
    }
    const turn: ActiveTurn = { child, canceled: false, sawCompletion: false, stderr: "" };
    chat.turn = turn;
    const stall = () => {
    ...
    child.on("error", (error) => {
      ...
      this.finishTurn(chat, "failed", missing ? `The chat provider "${config.command}" was not found on PATH.` : messageOf(error), missing ? "provider_missing" : "spawn_failed");
  ...
  private onProviderEvent(chat: ChatSession, turn: ActiveTurn, event: Record<string, unknown>) {
